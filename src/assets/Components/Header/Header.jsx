...
const Header = () => {
  ...
  return (
    <header
      ...
      ${
        isShrunk
          ? "top-2 py-2 shadow-md border-red-300"
          : "top-1 py-5 border-gray-200 shadow-sm"
      } custom_container`}
    >
      <div
        className={`header-wrapper px-2 w-full flex justify-between items-center flex-wrap  gap-4 transition-all duration-500 
      ...
      >
        ...
        <div
          className={`flex gap-0 sm:gap-2 items-center group cursor-pointer p-1 pr-3 rounded-xl transition-all duration-500 hover:bg-gray-50 
          ${isShrunk ? "scale-90" : "scale-100"}`}
        >
          <div className="relative">
            <img
              className="w-8 h-8 max-w-8 rounded-full border-2 border-green-500 p-0.5 object-cover
          ...
          <div className="flex flex-col">
            <span className="whitespace-nowrap hidden sm:block text-sm font-semibold text-gray-700 transition-all duration-300 group-hover:text-blue-600 group-hover:translate-x-1">
              profile_name
            </span>
